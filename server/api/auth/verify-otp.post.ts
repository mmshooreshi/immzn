...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    setCookie(event, 'auth_token', token, {
      ...
      maxAge: 60 * 60 * 24 * 7,
      // maxAge: 15, //debug short exp
    })
    $log.info('🍪 auth_token cookie 15 s')
    $log.debug('🔐 JWT set on cookie', token)
    $log.debug('📜 JWT payload', await verifyJwt(token))


    // ALSO issue a long-lived refresh_token (30 days)
    const refresh = await signRefreshJwt({ sub: user.id, phone, type: 'refresh' })
    setCookie(event, 'refresh_token', refresh, {
      ...
      // maxAge: 120     // debug short exp
    })
    $log.info('🍪 refresh_token cookie 120 s')
    $log.debug('🔐 JWT refresh token set on cookie', refresh)
    $log.debug('📜 JWT refresh token payload', await verifyJwt(refresh))
